...
launching_pca_osd_only<-function(){
  raw_control <-
    list.files(
      path = 'D:/Raw Resampled',
      pattern = '_control_',
      recursive = TRUE,
      full.names = TRUE
    )
  path = 'D:/Raw Resampled'
  for (control_file in raw_control){
    treatment_file<- sub("_control_", "_treatment_", control_file)
    control<-read.csv(control_file, row.names = 1)
    treated<-read.csv(treatment_file, row.names=1)

    control<-control[rowSums(control[])>0,]
    treated<-treated[rowSums(treated[])>0,]
    ...
    group_type<-sub(".*input_matrix_control_",'', control_file)
    group_type<-sub('.csv', '', group_type)
    output <-
      complete_howells_game(control_spectra, treated_spectra, group_type)
  }
}
...
complete_mcr_als<-function(matrix2D, component, type){
  ...
  osd_result <- osd(D = matrix2D, k = component, res.method = "ica.osd")
