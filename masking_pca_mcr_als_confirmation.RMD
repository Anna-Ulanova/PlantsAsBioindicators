...
grouping_VNIR_files<-function(directory){
  spectral_files<-list.files(path=directory, pattern='raw_rd_rf$', recursive=TRUE, full.names= TRUE)
  ...
  # group_split()
  return(dfgroup_summary)
}
```


```{r}
assigning_control_v_treated(grouped){
...
# 
# control_mcr<-complete_mcr_als(na_control, initial_comp,'Control')
# final_comp<-control_mcr$comp
# control_spectra<-control_mcr$spectra
# # control spectra: [1:59, 1:2]
# treated_mcr<-complete_mcr_als(na_treated, final_comp, 'Treated')
