---
title: "Big_Daddy_MCR_ALS_troubleshooting"
author: "Anna Ulanova"
date: "2024-08-29"
...
convert_to_2d<-function(hypercube){
  matrix2D<-matrix(hypercube, nrow = dim(hypercube)[1] * dim(hypercube)[2], ncol = dim(hypercube)[3])
  return(matrix2D)
}
```
...
```{r}
complete_mcr_als<-function(matrix2D, component){
  # Step 1: Perform MCR-ALS on control_2d
# Initialize random matrices for MCR-ALS
  initial_concentration_profiles <- matrix(runif(nrow(matrix2D) * component), 
                                                   nrow = nrow(matrix2D), 
                                                   ncol = component)
  ...
  reconstructed_spectra <- resolved_concentration %*% t(resolved_spectra)
  #
  ...
  plot(1:n_bands, matrix2D[10, ], type = "l", col = "red",
       xlab = "Wavelength/Band", ylab = "Intensity",
       main = "Comparison of Original and Reconstructed Spectra")
  lines(1:n_bands, reconstructed_spectra[10, ], col = "blue")
...
complete_howells_game<-function(control, treated){
  ...
  # Perform the Howells-Games test
  test_result <- games_howell_test(data)
...
treated_mcr<-complete_mcr_als(treated2d, final_comp)
treated_spectra<-treated_mcr$spectra
# treated spectra: [1:59,1]
complete_howells_game(control_spectra, treated_spectra)
```