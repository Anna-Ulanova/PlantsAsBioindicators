...
# Updates require to group by metal, treatment type and concentration AND BY SAMPLING DATE
# Sampling dates: GH_20231213, GH_20231121, GH_20231116, GH_20231114, GH_20230822, GH_20230801
# GH_20230726, GH_20230724

```{r Polished and debugged}
assigning_control_v_treated<-function(grouped){
  dates <- c("GH_20231213", "GH_20231121", "GH_20231116", "GH_20231114", 
           "GH_20230822", "GH_20230801", "GH_20230726", "GH_20230724")
  # Define the control groups and their corresponding treatments
  base_control_treatment <- list(
  ...
  # Initialize expanded lists
  control_treatment_mapping <- list()
  control_type_names_mapping <- list()
  
  # Generate lists for each date
  for (date in dates) {
    for (control in names(base_control_treatment)) {
      # Expand control_treatment_mapping
      new_control <- paste0(control, "_", date)
      control_treatment_mapping[[new_control]] <- paste0(base_control_treatment[[control]], "_", date)
      
      # Expand control_type_names_mapping
      control_type_names_mapping[[new_control]] <- paste0(base_control_type_names[[control]], "_", date)
    }
  }
